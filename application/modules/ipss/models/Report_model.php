...
class Report_model extends CI_Model
{
    ...
    function getStaffDispensingData($drug_id = null, $time_period = null)
    {
        ...
        if ($time_period) {
            switch ($time_period) {
                case 'day':
                    $this->db->where('TRUNC(T08_DISP_DATE) = TRUNC(SYSDATE)',null,false);
                    break;
                case 'month':
                    $this->db->where('EXTRACT(MONTH FROM T08_DISP_DATE) = EXTRACT(MONTH FROM SYSDATE)',null,false);
                    $this->db->where('EXTRACT(YEAR FROM T08_DISP_DATE) = EXTRACT(YEAR FROM SYSDATE)',null,false);
                    break;
                case 'year':
                    $this->db->where('EXTRACT(YEAR FROM T08_DISP_DATE) = EXTRACT(YEAR FROM SYSDATE)',null,false);
                    break;
